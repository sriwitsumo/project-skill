...
| 3 | สมมติฐาน (ถ้าจำเป็น) | คาดการณ์ผลล่วงหน้าไว้ว่าอย่างไร? | `reference/03-sommutithan.md` |
| 4 | ขอบเขตการศึกษา + แผนดำเนินงาน | ทำแค่ไหน ใคร อะไร ที่ไหน เมื่อไหร่? | `reference/04-khobkhet.md` |
| 5 | ประโยชน์ที่คาดว่าจะได้รับ | ใครได้อะไรหลังจบโครงงาน? | `reference/05-prayotchn.md` |
| 6 | นิยามศัพท์เฉพาะ | คำเฉพาะในเล่มนี้แปลว่าอะไรกันแน่? | `reference/06-niyamsap.md` |
...
## เกณฑ์ขั้นต่ำบังคับ (ใช้เป็นค่าเริ่มต้นเสมอ เว้นแต่ผู้ใช้/สถาบันระบุไว้ต่างจากนี้)

| หัวข้อ | เกณฑ์ขั้นต่ำ | อ้างอิงรายละเอียด |
|---|---|---|
| ที่มาและความสำคัญ | อย่างน้อย **2 หน้า A4 ฟอนต์ TH Sarabun New 16pt** เติมด้วยเนื้อหาจริง (หลักฐาน/การวิเคราะห์) ไม่ใช่คำฟุ่มเฟือย | `reference/01-thima-khwamsamkhan.md` หัวข้อ "ข้อกำหนดความยาวขั้นต่ำ" |
| วัตถุประสงค์ | อย่างน้อย **3 ข้อ** | `reference/02-watthuprasong.md` หัวข้อ "ความยาวและจำนวนที่เหมาะสม" |
| สมมติฐาน (ถ้ามี) | ตัวแปรต้น-ตาม-ควบคุมทุกตัวต้อง **สัมพันธ์กันจริงเชิงเหตุผล** ห้ามใส่ตัวแปรควบคุมเพียงเพื่อให้ครบฟอร์ม | `reference/03-sommutithan.md` หัวข้อ "กฎเหล็ก: ตัวแปรต้น ตัวแปรตาม และตัวแปรควบคุม ต้องสัมพันธ์กันจริง" |
...
เกณฑ์เหล่านี้เป็น **ขั้นต่ำที่ต้องทำให้ถึง** ไม่ใช่เพดานที่ห้ามเกิน — ถ้าเนื้อหาจริงของโครงงานมีมากกว่านี้ก็เขียน
ได้ตามจริง แต่ห้ามต่ำกว่าเกณฑ์นี้โดยไม่มีเหตุผล และห้ามเติมให้ถึงเกณฑ์ด้วยการพูดซ้ำ/คำฟุ่มเฟือย (ดูหลักการ
ถัดไป)
...
- **ความยาวไม่ใช่ตัวชี้วัดคุณภาพ แต่ต้องถึงเกณฑ์ขั้นต่ำข้างต้นด้วยเนื้อหาจริง** — บทที่ 1 ที่ดีคือกระชับ ตรงประเด็น
  ไม่ใช่ยาวที่สุดเท่าที่จะยาวได้ แต่ก็ต้องไม่สั้นกว่าเกณฑ์ขั้นต่ำที่สถานศึกษาไทยคาดหวัง วิธีทำให้ถึงเกณฑ์ที่ถูกต้อง
  คือเพิ่มหลักฐาน/การวิเคราะห์/มุมมองใหม่ ไม่ใช่พูดซ้ำเรื่องเดิมด้วยถ้อยคำต่างกัน ซึ่งเป็นสาเหตุอันดับหนึ่งของ
  อาการวกวน
...
- **ภาษาทางการเสมอ** — ไม่ใช้ภาษาพูด ไม่ใช้คำย่อที่ไม่เป็นทางการ ไม่ใช้สรรพนามบุรุษที่ 1/2 (เว้นแต่ใน
  โครงงานระดับต้น ๆ ที่ครูอนุญาตให้ใช้ "คณะผู้จัดทำ" แทนตัวเอง) ตรวจตาม checklist ใน
